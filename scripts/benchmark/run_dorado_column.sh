...
for entry in $DATASETS; do
    ...
    SRC=$(ls $BENCH/bacteria/$S/modbam/${S}_sup_v5r3_${TAG}*.bam 2>/dev/null | head -1)
    if [ -z "$SRC" ]; then echo "$LABEL: SKIPPED, no modbam matching ${S}_sup_v5r3_${TAG}*.bam (have: $(ls $BENCH/bacteria/$S/modbam/ 2>/dev/null | grep -c bam$) bams)" | tee -a $STATUS; continue; fi
    BED=$OUT/${S}_${TAG}.bed
